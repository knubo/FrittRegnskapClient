...
public class StandardvaluesView extends Composite implements ClickHandler,
	...
	private ListBoxWithErrorText initialIBPost;
	...
	private FlexTable setupAccountsTab() {
		...
		accountsTable.setHTML(6, 0, elements
				.setup_expected_income_or_cost_post());
		...
		accountsTable.setHTML(9, 0, elements.setup_init_ib_post());
		...
		initialIBPost = new ListBoxWithErrorText("initial_ib_post");
		accountsTable.setWidget(9, 1, initialIBPost);
		...
		editRegisterMembershipPostsImage = ImageFactory
				.editImage("register_membership_posts");
		...
		posttypeCache
				.fillAllPosts(initialIBPost.getListbox(), null, true, true);
	...
	public void onClick(ClickEvent event) {
		...
		if (event.getSource() == editFordringerImage) {
			AccountSelector instance = AccountSelector.getInstance(elements,
					constants, messages);
			instance.init(elements.setup_expected_income_or_cost_post(),
					getFordringerPost(), this);
			instance.center();
		}
		if (event.getSource() == editMonthTransferImage) {
			AccountSelector instance = AccountSelector.getInstance(elements,
					constants, messages);
			instance.init(elements.setup_end_month_transfer_posts(),
					getEndMonthTransferPosts(), this);
			instance.center();
		}
		if (event.getSource() == editRegisterMembershipPostsImage) {
			AccountSelector instance = AccountSelector.getInstance(elements,
					constants, messages);
			instance.init(elements.setup_register_membership_posts(),
					getRegisterMembershipPosts(), this);
			instance.center();
		}
	...
	private void save() {
		...
		Util.addPostParam(sb, "massletter_due_date", massletterDueDateBox
				.getText());
		Util.addPostParam(sb, "year_post", Util.getSelected(postYear
				.getListbox()));
		Util.addPostParam(sb, "course_post", Util.getSelected(postCourse
				.getListbox()));
		Util.addPostParam(sb, "train_post", Util.getSelected(postTrain
				.getListbox()));
		Util.addPostParam(sb, "youth_post", Util.getSelected(postYouth
				.getListbox()));
		Util.addPostParam(sb, "end_month_post", Util.getSelected(endMonthPost
				.getListbox()));
		Util.addPostParam(sb, "end_year_post", Util.getSelected(endYearPost
				.getListbox()));
		Util.addPostParam(sb, "ib_post", Util.getSelected(initialIBPost
				.getListbox()));
		Util.addPostParam(sb, "fordringer_posts", getFordringerPost());
		Util.addPostParam(sb, "end_month_transfer_posts",
				getEndMonthTransferPosts());
		Util.addPostParam(sb, "register_membership_posts",
				getRegisterMembershipPosts());
		Util.addPostParam(sb, "birthdate_required", birthdateRequiredBox
				.getValue() ? "1" : "0");
		...
		ServerResponse callback = new ServerResponse() {
			...
			public void serverResponse(JSONValue value) {
				...
				for (int i = 0; i < arr.size(); i++) {
					...
					semesterBox.addItem(obj.get("description"), obj
							.get("semester"));
				}
	...
	private void fillStandardValues() {
		ServerResponse callback = new ServerResponse() {
			...
			public void serverResponse(JSONValue value) {
				...
				massletterDueDateBox.setText(Util.str(object
						.get("massletter_due_date")));
				Util.setIndexByValue(postYear.getListbox(), Util.str(object
						.get("year_post")));
				Util.setIndexByValue(postCourse.getListbox(), Util.str(object
						.get("course_post")));
				Util.setIndexByValue(postTrain.getListbox(), Util.str(object
						.get("train_post")));
				Util.setIndexByValue(postYouth.getListbox(), Util.str(object
						.get("youth_post")));
				Util.setIndexByValue(endMonthPost.getListbox(), Util.str(object
						.get("end_month_post")));
				Util.setIndexByValue(endYearPost.getListbox(), Util.str(object
						.get("end_year_post")));
				Util.setIndexByValue(initialIBPost.getListbox(), Util
						.str(object.get("ib_post")));
				setFordringerPosts(Util.strSkipNull(object
						.get("fordringer_posts")));
				setEndMonthTransferPosts(Util.strSkipNull(object
						.get("end_month_transfer_posts")));
				setRegisterMembershipPosts(Util.strSkipNull(object
						.get("register_membership_posts")));
				birthdateRequiredBox.setValue(Util.getBoolean(object
						.get("birthdate_required")));
			}

		};

		AuthResponder.get(constants, messages, callback,
				"registers/standard.php?action=get");
	}
	...
	private boolean validate() {
		...
		masterValidator.mandatory(messages.required_field(), new Widget[] {
				yearBox, monthBox, semesterBox, endMonthPost, endYearPost,
				initialIBPost, postCourse, postTrain, postYear, postYouth });
		...
		masterValidator.date(messages.date_format(),
				new Widget[] { massletterDueDateBox });
