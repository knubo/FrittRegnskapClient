qx.Class.define("frittregnskapmedlemsportal.Membersearch",{
		...
		members : {
			...
			sendEmail : function(obj) {
				...
				var emailOp = new frittregnskapmedlemsportal.Email();
				emailOp.setupEmailWindow(this.__desktop, obj.p, obj.f);
			...
			filterUsers : function() {
				...
				try {
					...
					if (keywords.length > 0) {
						...
						for ( var i = 0; i < this.__users.length; i++) {
							...
							for ( var k = 0; k < keywords.length; k++) {
								if (user.f.toLowerCase().indexOf(keywords[k]) == -1
										&& user.l.toLowerCase().indexOf(keywords[k]) == -1) {
									found = false;
									break;
								}
							}
					...
					for ( var i = 0; i < listToIterate.length; i++) {
						...
						if (includeImages) {
							var itemWithImage = new qx.ui.form.ListItem(user.f + " " + user.l,
									"/RegnskapServer/services/portal/portal_persons.php?action=image&personId=" + user.p);
			...
			loadData : function() {
				var req = new qx.io.remote.Request("/RegnskapServer/services/portal/portal_persons.php?action=share", "GET",
						"application/json");

				req.addListener("completed", function(response) {
					this.__users = response.getContent();
					this.filterUsers();
				},this);
				req.send();
			...
			userList.addListener("dblclick", function(e) {
				...
				try {
					this.openProfileForUser(selection[0].getModel());
				} catch (error) {
					if (console) {
